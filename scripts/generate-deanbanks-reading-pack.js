...
// ── The Olivia Library ────────────────────────────────────────────────────────
// Olivia's own reference documents (strategic white paper, investor narrative,
// product architecture). These PDFs are pre-built and kept verbatim under
// data/netify/pdfs/olivia/ — they are NOT regenerated by this script, but are
// copied from their source location on first run so the pack stays complete.
const OLIVIA_PDFS = [
  { n: '11', title: 'Olivia — Strategic White Paper', time: 'The Olivia Library',
    desc: 'The vision for Olivia as an AI operating environment — the August 2026 strategic white paper on mission, philosophy and ecosystem design.',
    src: 'pdf/olivia/01-OLIVIA-Strategic-White-Paper-August-2026.pdf',
    href: 'pdfs/olivia/01-OLIVIA-Strategic-White-Paper-August-2026.pdf' },
  { n: '12', title: 'Olivia — Investor & Strategic Partner Narrative', time: 'The Olivia Library',
    desc: 'The investor and strategic-partner narrative — how Olivia positions itself for funding, partnerships and ecosystem growth.',
    src: 'pdf/olivia/02-OLIVIA-Investor-Strategic-Partner-Narrative.pdf',
    href: 'pdfs/olivia/02-OLIVIA-Investor-Strategic-Partner-Narrative.pdf' },
  { n: '13', title: 'Olivia — Product Architecture Document', time: 'The Olivia Library',
    desc: 'The product architecture document — identity, memory, AI orchestration and governance in the Olivia operating environment.',
    src: 'pdf/olivia/03-OLIVIA-Product-Architecture-Document.pdf',
    href: 'pdfs/olivia/03-OLIVIA-Product-Architecture-Document.pdf' },
];
...
// ── Olivia Library PDFs ───────────────────────────────────────────────────────
// The three Olivia reference PDFs are pre-built (not regenerated here). Ensure
// they live at data/netify/pdfs/olivia/: copy from the legacy pdf/olivia/
// location if present, otherwise skip if already in place.
function copyOliviaPdfs() {
  for (const doc of OLIVIA_PDFS) {
    const dst = path.join(PROJECT, 'data/netify', doc.href);
    if (fs.existsSync(dst)) {
      console.log('\u2713 olivia pdf present: ' + path.basename(dst));
      continue;
    }
    const src = path.join(PROJECT, 'data/netify', doc.src);
    if (fs.existsSync(src)) {
      fs.mkdirSync(path.dirname(dst), { recursive: true });
      fs.copyFileSync(src, dst);
      console.log('\u2713 olivia pdf copied: ' + path.basename(dst));
    } else {
      console.warn('\u26a0 olivia pdf missing (skipped): ' + dst);
    }
  }
}
...
function buildReadingGuide() {
  ...
  const oliviaOrderCards = OLIVIA_PDFS.map((doc, i) =>
    `<a class="order-card reveal" href="#olivia-0${i + 1}">
       <span class="n">${doc.n}</span>
       <span><span class="t">${escapeHtml(doc.title)}</span><br><span class="d">${escapeHtml(doc.desc)}</span><br><span class="d">${escapeHtml(doc.time)}</span><br><span class="dl"><i class="fas fa-file-pdf"></i> Open PDF</span></span>
     </a>`).join('\n');

  const oliviaIndexRows = OLIVIA_PDFS.map((doc, i) =>
    `<tr><td style="white-space:nowrap">${doc.n}</td><td><a href="#olivia-0${i + 1}">${escapeHtml(doc.title)}</a></td><td>${escapeHtml(doc.time)}</td><td><a class="dl ghost" style="background:transparent;border:1px solid var(--color-border-medium);color:var(--green-dark);font-size:.66rem;padding:.35rem .7rem" href="${doc.href}" target="_blank" rel="noopener"><i class="fas fa-book-open"></i> Open</a></td><td><a class="dl" style="background:var(--green-mid);font-size:.66rem;padding:.35rem .7rem" href="${doc.href}" download>PDF</a></td></tr>`
  ).join('\n');

  const oliviaCards = OLIVIA_PDFS.map((doc, i) =>
    `<article class="orig-card reveal" id="olivia-0${i + 1}">
      <h4>${escapeHtml(doc.title)}</h4>
      <p>${escapeHtml(doc.desc)}</p>
      <div class="doc-actions">
        <a class="dl ghost" href="${doc.href}" target="_blank" rel="noopener" title="Open in a new tab"><i class="fas fa-book-open"></i> Open PDF</a>
        <a class="dl" href="${doc.href}" download title="Download PDF"><i class="fas fa-file-pdf"></i> Download PDF</a>
      </div>
    </article>`
  ).join('\n');
...
<nav>
  ...
  <div class="links">
    ...
    <a href="#olivia-library" class="hide-sm">Olivia Library</a>
...
<section class="block" id="overview">
  ...
  <div class="markdown-body">
    ...
    <p>The pack is organised in five parts: <strong>Part A · Orientation</strong> (the group at a glance and the person behind it), <strong>Part B · The Group</strong> (venues, team, brands and partners), <strong>Part C · Events &amp; Brochures</strong> (private dining, weddings and the full brochures), <strong>Part D · The Olivia Alignment</strong> (the analysis of why the group and Olivia fit), and <strong>Part E · The Olivia Library</strong> (Olivia&rsquo;s own strategic, investor and architecture documents).</p>
...
<section class="block" id="order">
  ...
  <div class="order-grid">${orderCards}${oliviaOrderCards}</div>
</section>
...
<section class="block" id="index">
  ...
  <div class="markdown-body"><table>
    ...
    <tbody>${indexTable}${oliviaIndexRows}</tbody>
  </table></div>
</section>
...
<section class="block" id="olivia-library">
  <div class="sec-kicker">Part E · The Olivia Library</div>
  <h2 class="sec-title">Olivia&rsquo;s own documents</h2>
  <p class="lede" style="color:var(--gray-hi);font-size:.9rem;margin-bottom:1.4rem">Three Olivia reference documents complete the story — the strategic white paper, the investor &amp; strategic-partner narrative, and the product architecture document. Open them in the browser or download them.</p>
  <div class="orig-grid">${oliviaCards}</div>
</section>
...
async function main() {
  ...
  if (!skipPdfs) {
    ...
    console.log('skip-pdfs: existing PDFs kept as-is');
  }
  copyOriginals();
  copyOliviaPdfs();
  ...
  console.log('\u2728 ' + OLIVIA_PDFS.length + ' Olivia Library PDFs in ' + path.join(OUT_DIR, 'olivia'));
