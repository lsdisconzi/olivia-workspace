...
(function() {
  ...
  var _LOCAL_MAP_ROOT = (window.OliviaLegal_FUNCTIONS_MAP_ROOT || 'olivialegal_functions_mapping').replace(/\/$/, '');
  ...
  function init() {
    if (window.OliviaLegal_EMBED_MODE) return Promise.resolve();
  ...
  function _resolveUrl(tpl) {
    ...
    var dscServer = (typeof _DSC_SERVER !== 'undefined') ? _DSC_SERVER
      : (window.OliviaLegal_BASE_URLS && window.OliviaLegal_BASE_URLS.bridge)
        ? window.OliviaLegal_BASE_URLS.bridge
  ...
  function toSystemPrompt() {
    ...
    // Runtime API hints
    p += '### Runtime API\n';
    p += 'Use `OliviaLegal_FUNCTIONS.matchEndpoint(method, url)` to identify any API call.\n';
    p += 'Use `OliviaLegal_FUNCTIONS.loadFunction(id)` for full schemas (body, params, response).\n';
    p += 'Use `OliviaLegal_FUNCTIONS.search(query)` to find functions by keyword.\n';
    p += 'Use `OliviaLegal_FUNCTIONS.getRelated(id)` to navigate function relationships.\n';
